...
namespace SwcBuild
{
    ...
    {
        public const string AirSdkDescription = "air-sdk-description.xml";
        public const string FlexSdkDescription = "flex-sdk-description.xml";
        ...
        public ExitCodes Run(string[] args)
        {
            try
            {
                ...
                if (WriteConfigXml(ObjPath, out bool configXmlWritten, out exitCode)) return exitCode;

                WriteLineAssert($"Configuration file created: {Compiler.ConfigFile}", configXmlWritten);
        ...
        private bool GetTargetPlayer(out ExitCodes exitCode)
        {
            ...
            if (buildOptions.MinorVersion2.Trim().Length > 0)
                version += buildOptions.MinorVersion2.Trim();
            else
                version += buildOptions.MinorVersion.Trim();

            if (!Version.TryParse(version, out var targetPlayer))
                return ErrorHelper.InvalidFormat(out exitCode, $@"{ProjectPath}\version", version);

            if (targetPlayer > sdkVersion)
            {
                WriteLine($"Target player version {targetPlayer} is higher than the SDK version {sdkVersion} and automatically lowered to match the SDK version.");
                targetPlayer = sdkVersion;
            }
        ...
        private bool WriteConfigXml(string directory, out bool written, out ExitCodes exitCode)
        {
